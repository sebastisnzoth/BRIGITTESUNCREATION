...
- Nenhum agente pode gerar gasto, contratar serviço pago ou ativar cobrança sem autorização humana explícita.
- Se uma ferramenta gratuita deixar de servir, migrar para alternativa gratuita preservando dados e portabilidade.
- Receita futura deve, sempre que possível, financiar a própria expansão do projeto.

## Regra 1 — A obra é inviolável
A tecnologia serve à obra. A obra não serve à tecnologia.

Nenhum agente pode reescrever, corrigir, resumir, cortar, adaptar, traduzir para publicação ou alterar a obra sem autorização explícita de Brigitte. Trabalhos técnicos devem ocorrer em cópias. O original deve permanecer preservado.

## Regra 2 — Brigitte decide
...
1. Isso preserva a obra?
2. Isso protege os interesses de Brigitte?
3. Pode ser feito com custo zero agora?
4. Aumenta alcance, independência ou sustentabilidade?
5. É reversível e auditável?
...
- `HANDOFF.md`: estado operacional.
- `.agents/skills/*/SKILL.md`: especialidades da equipe digital.

## Estados de trabalho
`NEXT` · `IN PROGRESS` · `IMPLEMENTED` · `VALIDATED` · `RELEASED` · `BLOCKED`
